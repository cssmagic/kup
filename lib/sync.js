...
async function postIssue(fileInfo, repo, options = {}) {
	...
	let shouldPostNewIssue = options.yes
	if (!shouldPostNewIssue) {
		...
		shouldPostNewIssue = answer.postNewIssue
	}
...
async function confirmWriteIssueMeta(file, { yes = false } = {}) {
	if (yes) return true
...
async function confirmOverwriteDumpFile(file, { yes = false } = {}) {
	if (yes) return true
